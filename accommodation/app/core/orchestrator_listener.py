            ...
            if message.value['action'] == 'commit':
                ...
                if message.value['item']:
                    items = await Accommodation.find(
                        Accommodation.host_id == uuid.UUID(message.value['item'])).to_list()
                    if len(items) == 0:
                        ...
                        for item in items:
                            ...
                            logger.success("Deleted Accomodation successfully saved")
                            # produce success message
                            kafka_producer.send('orchestrator-responses', {
                                'transaction_id': str(message.value['transaction_id']),
                                'source': 'accommodation',
                ...
                logger.info("Received rollback message")
                deleted_accommodations = await DeletedAccommodation.find(
                    DeletedAccommodation.transaction_id == uuid.UUID(message.value['transaction_id'])).to_list()
