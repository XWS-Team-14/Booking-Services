    ...
    def convertDto(request):
        ...
        if request.occupied_intervals:
            for item in request.occupied_intervals:
                ocuppied_intervals_list.append(Interval(date_start=AvailabilityHelper.convertDate(item.date_start),
                                                        date_end=AvailabilityHelper.convertDate(item.date_end)))

        return Availability(
            id=uuid.UUID(request.availability_id),
            accomodation_id=request.accomodation_id,
            available_interval=Interval(date_start=AvailabilityHelper.convertDate(request.interval.date_start),
    ...
    def calculatePrice(requested_interval, num_of_guests, availability, holidays):
        ...
        if not availability.special_pricing:
            ...
            for day_num in range(
                    (requested_interval_date.date_end.date() - requested_interval_date.date_start.date()).days + 1):
